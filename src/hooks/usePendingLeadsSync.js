...
// No hard retry cap: a long outage / temporarily-bad secret must not permanently
// strand a real lead. Backoff is clamped at MAX_DELAY, so a stuck lead just
// retries politely (~once/min) until it lands. retryCount still grows (used
// only for backoff).
function isDue(rec) {
...
// Mounted once on the public site: any lead this browser queued but could not
// deliver is resent on the next page load, on reconnect, or when the tab
// regains focus.
