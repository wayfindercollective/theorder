 ...
 * The relay or Wayfinder OS may be slow / cold-starting / down — we never lose a lead.
 *
 * `_v2` key: the first generation of this queue (removed 2026-08-03 with the
 * Instagram handoff) used `order_pending_leads`. A browser that failed to
 * deliver a lead back then may still hold one under that key; reading it now
 * would resend a months-old application as if it were new. The new key leaves
 * those stale records untouched.
 */

const KEY = 'order_pending_leads_v2'
...
// always being attempted, this closes the "storage AND relay both fail" gap.
let memFallback = {}
